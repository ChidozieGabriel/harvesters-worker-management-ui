...
import { Plus, Edit2, Trash2, Save, X, Building2, Users, Briefcase } from 'lucide-react';
import { getAllDepartments, createDepartment, updateDepartment, deleteDepartment, getTeams } from '../../services';
import { Department, Team } from '../../types/api';
...
export default function Departments() {
  ...
  // Group departments by team for better organization
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6 lg:gap-8">
        <div className="space-y-3 lg:space-y-4 min-w-0 flex-1">
          ...
          <p className="text-base lg:text-lg text-harvesters-600 leading-relaxed">
            Organize church departments within teams to structure ministry operations
          </p>
        </div>
        ...
        <div className="flex-shrink-0">
          <button
            ...
            disabled={isCreating || editingDepartment}
          ...
          <div className="card-padding">
            <form onSubmit={handleSubmit(onSubmit)} className="form-spacing">
              ...
              <div className="space-y-2 lg:space-y-3">
                ...
                <select
                  ...
                  className={`appearance-none block w-full button-padding border rounded-xl lg:rounded-2xl shadow-sm text-sm lg:text-base
                    focus:outline-none focus:ring-2 focus:ring-harvesters-500 focus:border-transparent transition-all duration-200
                    ${errors.teamName ? 'border-red-300 bg-red-50' : 'border-gray-300 hover:border-harvesters-300'}`}
                  {...register('teamName', { 
                ...
                {errors.teamName && (
                  <p className="text-sm text-red-600 flex items-center">
                    <span className="mr-2">⚠</span>
                    <span className="truncate">{errors.teamName.message}</span>
                  </p>
                )}
                {teams.length === 0 && (
                  <p className="text-sm text-amber-600 flex items-center">
                    <span className="mr-2">⚠</span>
                    <span className="truncate">No teams available. Create a team first.</span>
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 lg:gap-4 pt-4 lg:pt-6">
                <button
                  type="submit"
                  disabled={createDepartmentMutation.isPending || updateDepartmentMutation.isPending || teams.length === 0}
      ...
      <div className="bg-white rounded-2xl lg:rounded-3xl shadow-lg border border-harvesters-100 overflow-hidden">
        <div className="card-padding border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 lg:space-x-6">
              ...
              <div>
                <h2 className="text-xl lg:text-2xl font-bold text-gray-900">
                  Church Departments ({departments.length})
                </h2>
                <p className="text-sm text-harvesters-600 mt-1">
                  Manage and organize your ministry departments
                </p>
              </div>
            </div>
          </div>
        ...
        {departments.length === 0 ? (
          <div className="card-padding text-center py-12 lg:py-16">
            <Building2 className="w-12 h-12 lg:w-16 lg:h-16 text-gray-400 mx-auto mb-4 lg:mb-6" />
            ...
            <p className="text-sm lg:text-base text-gray-600 mb-6 lg:mb-8 max-w-md mx-auto">
              Create your first department to start organizing church ministries within teams.
            </p>
            {teams.length > 0 ? (
              <button
                onClick={() => setIsCreating(true)}
              ...
              <div className="space-y-4">
                <p className="text-sm text-amber-600">
                  You need to create at least one team before adding departments.
                </p>
                <button
                  onClick={() => window.location.href = '/admin/teams'}
                  className="inline-flex items-center button-padding bg-amber-600 text-white rounded-xl lg:rounded-2xl hover:bg-amber-700 transition-all duration-200 shadow-lg hover:shadow-xl font-medium text-sm lg:text-base"
                >
                  <Briefcase className="w-4 h-4 lg:w-5 lg:h-5 mr-2 lg:mr-3" />
                  Go to Teams
                </button>
              </div>
            )}
          ...
          <div className="space-y-6 lg:space-y-8">
            {Object.entries(departmentsByTeam).map(([teamName, teamDepartments]) => (
              <div key={teamName} className="card-padding border-b border-gray-100 last:border-b-0">
                {/* Team Header */}
                <div className="flex items-center space-x-3 lg:space-x-4 mb-4 lg:mb-6">
                  <div className="bg-harvesters-400 p-2 lg:p-3 rounded-lg lg:rounded-xl text-white">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-harvesters-600">
                      {teamDepartments.length} {teamDepartments.length === 1 ? 'Department' : 'Departments'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid-responsive grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                  {teamDepartments.map((department) => (
                    <div key={department.id} className="bg-gray-50 border border-gray-200 rounded-xl lg:rounded-2xl hover:border-harvesters-200 hover:shadow-lg transition-all duration-300 group">
                      <div className="card-padding">
                        {/* Department Header */}
                          ...
                          <div className="flex items-center justify-between pt-3 lg:pt-4 border-t border-gray-200">
                            ...
                            <div className="text-right">
                              <p className="text-xs lg:text-sm text-harvesters-600 font-semibold">
                                ID: {department.id.slice(-8).toUpperCase()}
                              </p>
                            </div>
                          </div>
                        </div>
